...
if __name__ == "__main__":
    ...
    for i in range(10):
        s = Sphere( comment="random"+str(i),
                    center=[random.randrange(-10,10),random.randrange(-10,10),random.randrange(-10,10)],
                    radius=random.random()*2+0.1,
                    material=Material(color = [random.random(),random.random(),random.random()],
                    reflectType = random.choice(list(reflectType))))
        objects.append(s)
    ...
    #box
    b = Box(comment="the box", minCorner=[-25, -30, 10], maxCorner=[-15, -10, 20],
            material=Material(color=[0.3, 0.4, 0.4]))
    objects.append(b)
